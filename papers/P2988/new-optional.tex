...
namespace std {
  ...
  @\added{// \ref{optional.optional.ref}, class template \tcode{optional} for reference types }@
...
or a complete non-array object type, and \tcode{remove_cvref<X>} is a type other than \tcode{in_place_t} or \tcode{nullopt_t}.
...
When an \tcode{optional<T\&>} contains a value, the \defnx{contained value}{contained value!\idxcode{optional.ref}} is a reference to the \tcode{T} object that \exposid{val} points to.
...
\begin{itemdescr}
  ...
  \pnum
  \effects
  Equivalent to: \tcode{\exposid{convert-ref-init-val}(arg);}
...
\begin{itemdescr}
  ...
  \pnum
  \effects
  Equivalent to: \tcode{\exposid{convert-ref-init-val}(u);}
...
is_nothrow_constructible_v<T&, U>
  ...
  \tcode{reference_constructs_from_temporary_v<T\&, U>}
  is \tcode{true}
\end{itemdescr}
...
is_nothrow_constructible_v<T&, const U&>
  \end{codeblock}
  This constructor is defined as deleted if
\tcode{reference_constructs_from_temporary_v<T\&, const U\&>}
  is \tcode{true}
\end{itemdescr}
...
\begin{itemdescr}
  ...
  \mandates
  \begin{itemize}
    \item \tcode{is_constructible_v<remove_cv_t<T>, T\&>} is \tcode{true}.
    \item \tcode{is_convertible_v<U, remove_cv_t<T>>} is \tcode{true}.
  \end{itemize}
  ...
  Equivalent to:
  \begin{codeblock}
return has_value() ? *@\exposid{val}@ : std::forward<U>(u);
  \end{codeblock}
\end{itemdescr}
...
\begin{itemdescr}
  ...
  \pnum
  \mandates
  \tcode{U} is a valid contained type for optional.
